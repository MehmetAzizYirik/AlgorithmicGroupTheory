...
public class MORGEN {
    ...
    public int[] connectivityIndices = new int[2];
    ...
    public ThreadLocal<ArrayList<ThreadLocal<ArrayList<Permutation>>>> formerPermutations =
            ThreadLocal.withInitial(() -> new ArrayList<ThreadLocal<ArrayList<Permutation>>>());
    public int[] degrees;
    public int[] initialDegrees;
    public ThreadLocal<ArrayList<Integer>> initialPartition =
            ThreadLocal.withInitial(() -> new ArrayList<Integer>());
    ...
    public ThreadLocal<ArrayList<ThreadLocal<ArrayList<Integer>>>> partitionList =
            ThreadLocal.withInitial(() -> new ArrayList<ThreadLocal<ArrayList<Integer>>>());
    ...
    public int y = 0;
    public int z = 0;
    public List<int[][]> output = new ArrayList<int[][]>();
    ...
    /** Molecule depiction */
    public void depict(IAtomContainer molecule, String path)
    ...
    public boolean canBuildGraph(String formula) {
        boolean check = true;
        String[] atoms = formula.split("(?=[A-Z])");
        String[] info;
        String symbol;
    ...
    public boolean equalBlockCheck(
            int index, int y, int[][] A, Permutation cycleTransposition, Permutation perm) {
        boolean check = true;
        int[] canonical = A[index];
    ...
    public void setLernenIndices(
        ...
        nonCanonicalIndices.set(upperIndex(rowIndex1, rowIndex2, A, permutation));
    }
    ...
    public boolean firstCheck(int index, int[][] A, ArrayList<Integer> partition) {
        ...
        if (partition.size() != size) {
            if (!descendingOrdercomparison(index, partition, A[index])) {
                ...
                learningFromCanonicalTest = true;
                nonCanonicalIndices.set(upperIndex(index, index, A, canonicalPermutation));
            }
        }
        return check;
    }
    ...
    /**
     * As explained in Grund 3.4.1; we need to calculate upperIndex. First, we need our j index
     * where the original row become smaller, then calculating the lower index. Based on these two
     * values and the value of j in the permutation, we calculate our upper index.
     *
    ...
    public boolean descendingOrderUpperMatrix(
     ...
     * @throws CDKException
     */
    public void generate(int[] degreeList)
        ...
        while (flag) {
            ...
            if (learningFromConnectivity) {
                ...
                if (learningFromCanonicalTest) {
                    indices = successor(nonCanonicalIndices.get(), max.length);
                    findR(indices);
                    learningFromCanonicalTest = false;
                    callForward = false;
                }
     ...
     * @throws CDKException
     */
    public int[][] nextStep(int[][] A, int[] indices)
            throws IOException, CloneNotSupportedException, CDKException {
        if (callForward) {
     ...
     * @throws CDKException
     */
    public int[][] backward(int[][] A, int[] indices)
            throws IOException, CloneNotSupportedException, CDKException {
        int i = indices[0];
        int j = indices[1];
        if (i == 0 && j == 1) {
     ...
     * @throws CDKException
     */
    public int[][] forward(int[][] A, int[] indices)
            throws IOException, CloneNotSupportedException, CDKException {
        int i = indices[0];
        int j = indices[1];
        ...
        /**
         * if(j==(max.length-1)) { int[][] newMat = forward(lInverse, cInverse, maximumValue, i,
         * j,A,indices); return newMat; }else { return forward(lInverse, cInverse, maximumValue, i,
         * j,A,indices); }*
         */
    }
    ...
    protected MORGEN clone() {
        ...
        morgen.nonCanonicalIndices = nonCanonicalIndices;
        morgen.learningFromCanonicalTest = learningFromCanonicalTest;
        morgen.atomContainer = atomContainer;
        morgen.size = size;
        morgen.learningFromConnectivity = learningFromConnectivity;
    ...
    public boolean connectivityTest(int[][] mat) {
        ...
        if (zValue == 0 && allIs0(kValues)) {
            ...
            setLearningFromConnectivity(mat, zValues, kValues);
        }
        return check;
    }
     ...
     */
    public void setLearningFromConnectivity(int[][] mat, Set<Integer> zValues, int[] kValues) {
        learningFromConnectivity = true;
        ...
        for (int i = y; i <= z; i++) {
            test =
                    rowCanonicalTest(
                            ...
                            r.get(),
                            A,
                            partitionList.get().get(i).get(),
                            canonicalPartition(i, partitionList.get().get(i).get()));
            if (!test) {
                check = false;
                break;
            }
    ...
    public void clearFormers(boolean check, int y) {
        if (check == false) {
            ThreadLocal<ArrayList<ThreadLocal<ArrayList<Permutation>>>> newPerms =
                    ThreadLocal.withInitial(
                            () -> new ArrayList<ThreadLocal<ArrayList<Permutation>>>());
            ThreadLocal<ArrayList<ThreadLocal<ArrayList<Integer>>>> newPart =
                    ThreadLocal.withInitial(() -> new ArrayList<ThreadLocal<ArrayList<Integer>>>());
            for (int i = 0; i < y; i++) {
     ...
     * @param index int row index
     * @param y int first row of the tested block
     * @param total int total number of atoms
    ...
    public void candidatePermutations(int index, int y, int total, List<Permutation> cycles) {
        ThreadLocal<ArrayList<Permutation>> newList =
                ThreadLocal.withInitial(() -> new ArrayList<Permutation>());
        for (Permutation cycle : cycles) {
    ...
    public void addPartition(int index, ArrayList<Integer> newPartition, int[][] A) {
        ThreadLocal<ArrayList<Integer>> refinedPartition =
                ThreadLocal.withInitial(() -> new ArrayList<Integer>());
     ...
     * @return int[]
     */
    public int[] getCanonicalPermutation(ArrayList<Integer> partition, int[] max, int[] check) {
    ...
    public Permutation getCanonicalCycle(
        ...
        if (!equalBlockCheck(index, y, A, cycleTransposition, canonicalPermutation)) {
            canonicalPermutation =
                    getEqualPerm(cycleTransposition, index, y, total, A, newPartition);
            int[] check = row2compare(index, A, cycleTransposition);
    ...
    public static void main(String[] args)
        ...
        try {
            ...
            if (Objects.nonNull(gen.formula)) {
                ...
                List<Line> lines =
                        Files.lines(Paths.get(gen.formulaList))
                                .map(
                                        formula -> {
                                            double average =
                                                    IntStream.rangeClosed(1, 5)
                                                            .asLongStream()
                                                            .map(i -> execMorgen(gen, formula))
                                                            .average()
                                                            .getAsDouble();
